 ...
 *
 * Плюсы:
 *  - Атомарная передача сообщений, гарантированный порядок для каждого приоритета. (атомараная передача - это все или ничего)
 *  - Встроенные приоритеты: срочные сообщения читаются первыми.
 *  - Стандартизованный и надежный механизм IPC в пределах одного хоста.
 *
 * Минусы:
 *  - Жесткие ограничения на размер и количество сообщений (системные лимиты).
 *  - Медленнее shared memory из-за системных вызовов.
 *  - Очереди нужно явно создавать/удалять (mq_open/mq_unlink).
 */
